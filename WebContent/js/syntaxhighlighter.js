 ...
 * 
 * @version
 * 4.0.1 (Wed, 27 Apr 2016 14:23:42 GMT)
 * 
 * @copyright
...
/***/ function(module, exports) {
	...
	function getLinesToHighlight(lines) {
	  var results = {},
	      linesToHighlight,
	      l,
	      i;
	...
	function getHighlightType(lines, types){
		var results = {};
		
		lines = lines || [];
		types = types || [];
		
		if (typeof lines.push !== 'function') lines = [lines];
		if (typeof types.push !== 'function') types = [types];
		
		for (var i = 0, l = lines.length, k = types.length; i < l, i < k; i++) {
		    results[lines[i]] = types[i];
		}
		return results;
	}
	...
	function Renderer(code, matches, opts) {
	  ...
	  /** Added Code */
	  _this.linesToHighlight = getLinesToHighlight(opts.highlight);
	  _this.highlightType = getHighlightType(opts.highlight, opts.highlightType);
	...
	Renderer.prototype = {
	   ...
	   */
	  figureOutDisplayLineNumbers: function figureOutDisplayLineNumbers(code) {
	    var lineNumbers = [],
	        lines = this.lines,
	    ...
	    if(this.opts.skip){
	    	linesToSkip = this.opts.skip;
	    	if (typeof linesToSkip.push !== 'function'){
	    		linesToSkip = [linesToSkip];
	    	}
	    	
	    	for (i = 0, l = linesToSkip.length; i < l; i++) {
	    		skips[linesToSkip[i]] = true;
	    	}
	    }
	   ...
	   */
	  wrapLine: function wrapLine(lineIndex, lineNumber, lineHtml, isGutter) {
	    var classes = ['line', 'number' + lineNumber, 'index' + lineIndex, 'alt' + (lineNumber % 2 == 0 ? 1 : 2).toString(), this.opts.docType, isGutter ? 'gutter' : 'code'];
	
	    /** Added Code */
	    if (this.linesToHighlight[lineNumber]){
	    	var type = this.highlightType[lineNumber];
	    	if(!type){
	    		classes.push('highlighted_gray');  // default grayed out
	    	}else {
	    		switch(type){
	    			case "LINE_DELETED":
	    				classes.push('highlighted_delete');
	    				break;
	    			case "LINE_ADDED":
	    				classes.push('highlighted_add');
	    				break;
	    			case "LINE_DIFF":
	    				classes.push('highlighted_diff');
	    				break;
	    			case "LINE_SKIP":
	    			default:
	    				classes.push('highlighted_gray');
	    		}
	    	}
	    }
	    ...
	    var id = '';
	    if(!isGutter){
	    	id = 'id="number'+lineNumber+'_'+this.opts.docType+'_code"';
	    }
	
	    if(isGutter && this.opts.hideLineNumber){
	    	lineHtml = '  ';
	    }
	    return '<div class="' + classes.join(' ') + '" '+id+'>' + lineHtml + '</div><div class="line number'+lineNumber+' '+this.opts.docType+'_placeholder '+(isGutter ? 'gutter' : 'code')+'"></div>';
	  },
	
	  /**
	   * Generates HTML markup for line number column.
	   * @param {String} displayLineNumbers
	   * @return {String}       Returns HTML markup.
	   */
	  renderLineNumbers: function renderLineNumbers(displayLineNumbers) {
	    var _this = this,
	        opts = _this.opts,
	        html = '',
	    ...
	    for (i = 0; i < count; i++) {
	      ...
	      displayLineNumer = displayLineNumbers ? displayLineNumbers[i] : lineNumber;
	      code = displayLineNumer == 0 ? opts.space : padNumber(displayLineNumer, pad);
	      html += _this.wrapLine(i, lineNumber, code, true);
	    }
	   ...
	   */
	  getCodeLinesHtml: function getCodeLinesHtml(html) {
	    ...
	    for (var i = 0, l = lines.length; i < l; i++) {
	      ...
	      html += _this.wrapLine(i, lineNumber, (spaces != null ? '<code class="' + brushName + ' spaces">' + spaces + '</code>' : '') + line, false);
	    }
	  ...
	  getHtml: function getHtml() {
	    ...
	    if (gutter) displayLineNumbers = _this.figureOutDisplayLineNumbers(code);
	
	    // processes found matches into the html
	    html = _this.getMatchesHtml(code, matches);
	
	    // finally, split all lines so that they wrap well
	    html = _this.getCodeLinesHtml(html);
	
	    // finally, process the links
	    if (opts.autoLinks) html = _this.processUrls(html);
	
	    html = '\n      <div class="' + classes.join(' ') + '">\n        <table border="0" cellpadding="0" cellspacing="0">\n          ' + _this.getTitleHtml(opts.title) + '\n          <tbody>\n            <tr>\n              ' + (gutter ? '<td class="gutter">' + _this.renderLineNumbers(displayLineNumbers) + '</td>' : '') + '\n              <td class="code">\n                <div class="container">' + html + '</div>\n              </td>\n            </tr>\n          </tbody>\n        </table>\n      </div>\n    ';
